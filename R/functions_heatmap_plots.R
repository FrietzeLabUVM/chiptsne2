...
.annotation_theme = theme(panel.background = element_blank(), axis.ticks = element_blank(), axis.text = element_blank(), axis.title = element_blank())
    ...
    if(show_labels){
        for(i in seq_len(nrow(df_rects))){
            p = p + annotate("text",
                             ...
                             color = text_colors[rev(rownames(df_rects))[i]], #df_rects$color[i],
                             angle = label_angle)
        }
    }
    if(!show_legend){
...
row2 = cowplot::plot_grid(
    p_overlap.leg, p_cluster_bar.leg, p_cluster_bar2.leg, p_heat.leg, nrow = 1
)
...
.plotSignalHeatmap = function(ct2,
    ...
    clust_dt[[ct2@region_VAR]] = factor(clust_dt[[ct2@region_VAR]], levels = rev(levels(prof_dt[[ct2@region_VAR]])))
...
.plotSignalHeatmap(ct2, group_VARS = "cluster")
