...
The history of any ChIPtsne2 object can be used to rerun complete workflows for convenience and reprodcibility.
...
```{r}
prof_gr = seqsetvis::ssvFetchBam(bam_file_dt[1:2], query_gr, fragLens = 200)
ct2_tidy = ChIPtsne2.from_tidy(prof_gr, query_gr, name_VAR = "group")
# you could also do any normalization or other transformations before
prof_df = prof_gr %>% 
    as.data.frame %>%
    mutate(RPM = y / mapped_reads * 1e6)
# undebug(ChIPtsne2.from_tidy)
ct2_tidy2 = ChIPtsne2.from_tidy(prof_df, query_gr, value_VAR = "RPM", name_VAR = "group")
```
